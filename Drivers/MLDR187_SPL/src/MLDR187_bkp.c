...
void BKP_Init(BKP_InitTypeDef* initStruct)
{
	...
	MDR_BKP->PREDIV_S = (initStruct->RTCprediv) & BKP_PREDIV_PRL_Msk;
	MDR_BKP->PRL = (initStruct->RTCcounter) & BKP_PREDIV_PRL_Msk;
...
FlagStatus BKP_GetSingleFlag(bkpFlags flag)
{
	...
	/* test input parameters */
	assert_param(BKP_IS_SINGLE_FLAG);
...
static void BKP_GetDateTimeReg(uint32_t dateRegVal, uint32_t timeRegVal, RTC_DateTime* dateTime)
{
	...
	dateTime->month = (dateRegVal >> BKP_RTC_DR_MU_Pos) & 0xFFU;
	dateTime->day = (dateRegVal >> BKP_RTC_DR_DU_Pos) & 0xFFU;

	dateTime->hours = (timeRegVal >> BKP_RTC_TR_HU_Pos) & 0xFFU;
	if ((timeRegVal & BKP_RTC_TR_PM) != 0) {
		dateTime->pmFlag = SET;
	} else {
		dateTime->pmFlag = RESET;
	}
	dateTime->minutes = (timeRegVal >> BKP_RTC_TR_MNU_Pos) & 0xFFU;
	dateTime->seconds = (timeRegVal >> BKP_RTC_TR_SU_Pos) & 0xFFU;
}
